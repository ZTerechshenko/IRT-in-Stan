...
response <- Mobility[sample(1:nrow(Mobility), 20,
                          replace=FALSE),]# Running an IRT model in Stan can be expensive time-wise.
#For the purpose of this vignette, I limit my sample to 20 observations chosen randomly from the dataset
```
...
Now can draw the traceplot to inspect the sampling behavior and assess mixing across chains and convergence:

``` r
traceplot(fit, pars = "beta", inc_warmup = T, nrow = 2)
```

![](IRT_in_Stan_files/figure-markdown_github/unnamed-chunk-9-1.png)
